...
MFLevel::MFLevel( const godot::String &path ) :
    ...
    m_skyMaterial( memnew( godot::PanoramaSkyMaterial ) ), m_minViewDuration( 0.5 ),
...
void MFLevel::_enter_tree()
{
    ...
    if ( !m_editorMode )
    {
        ...
        m_background->set_cast_shadows_setting(
        ...
        godot::String shader = godot::FileAccess::get_file_as_string(
            "/Users/savanozin/Documents/MyFantasyGodot/demo/background.gdshader" );
...
void MFLevel::setup_cameras()
{
    if ( m_editorMode )
    {
        ...
        {
            MFViewData viewData = MFManager::get()->get_view_data( i );

            godot::Camera3D *camera = memnew( godot::Camera3D() );
            camera->set_name( viewData.name );
            camera->set_transform( viewData.transform );
            ...
            godot::UtilityFunctions::print( "made camera ", viewData.name );
        }
    }
}
...
bool MFLevel::set_view( int viewId )
{
    ...
    MFViewData viewData = MFManager::get()->get_view_data( viewId );

    if ( m_background == nullptr || m_gameCamera == nullptr || viewData.colorBuffer == nullptr ||
         viewData.depthBuffer == nullptr || viewData.envBuffer == nullptr )
    {
        return false;
    }

    godot::Ref<godot::Image> colorImage =
        godot::Image::create( viewData.sizeX, viewData.sizeY, false, godot::Image::FORMAT_RGBF );
    {
        godot::PackedByteArray data = colorImage->get_data();
        float *imageDst = reinterpret_cast<float *>( const_cast<uint8_t *>( data.ptr() ) );
        memcpy( imageDst, viewData.colorBuffer,
                viewData.sizeX * viewData.sizeY * 3 * sizeof( float ) );
    }

    godot::Ref<godot::Image> depthImage =
        godot::Image::create( viewData.sizeX, viewData.sizeY, false, godot::Image::FORMAT_RF );
    {
        godot::PackedByteArray data = depthImage->get_data();
        float *imageDst = reinterpret_cast<float *>( const_cast<uint8_t *>( data.ptr() ) );
        memcpy( imageDst, viewData.depthBuffer,
                viewData.sizeX * viewData.sizeY * 1 * sizeof( float ) );
    }

    godot::Ref<godot::Image> envImage =
        godot::Image::create( 1024, 512, false, godot::Image::FORMAT_RGBF );
    {
        godot::PackedByteArray data = envImage->get_data();
        float *imageDst = reinterpret_cast<float *>( const_cast<uint8_t *>( data.ptr() ) );
        memcpy( imageDst, viewData.envBuffer, 1024 * 512 * 3 * sizeof( float ) );
    }

    m_backgroundMaterial->set_shader_parameter(
        "color", godot::ImageTexture::create_from_image( colorImage ) );
    m_backgroundMaterial->set_shader_parameter(
        "depth", godot::ImageTexture::create_from_image( depthImage ) );

    m_skyMaterial->set_panorama( godot::ImageTexture::create_from_image( envImage ) );

    m_gameCamera->set_current( true );
    m_gameCamera->set_transform( viewData.transform );
    m_gameCamera->set_fov( viewData.fov );

    godot::UtilityFunctions::print( "set camera: ", viewData.name );
...
void MFLevel::_bind_methods()
{
    ...
    godot::ClassDB::bind_method( godot::D_METHOD( "set_closest_view", "point" ),
                                 &MFLevel::set_closest_view );

    godot::ClassDB::bind_method( godot::D_METHOD( "set_min_view_duration", "time ms" ),
                                 &MFLevel::set_min_view_duration );
    godot::ClassDB::bind_method( godot::D_METHOD( "get_min_view_duration" ),
                                 &MFLevel::get_min_view_duration );

    godot::ClassDB::bind_method(
        godot::D_METHOD( "set_level_file_path", "relative path to level file" ),
        &MFLevel::set_level_file_path );
    godot::ClassDB::bind_method( godot::D_METHOD( "get_level_file_path" ),
                                 &MFLevel::get_level_file_path );

    // Properties.
    ADD_GROUP( "View Properties", "group_" );
    ADD_PROPERTY( godot::PropertyInfo( godot::Variant::FLOAT, "group_min_view_duration" ),
                  "set_min_view_duration", "get_min_view_duration" );
    ADD_PROPERTY( godot::PropertyInfo( godot::Variant::STRING, "group_level_file_path" ),
