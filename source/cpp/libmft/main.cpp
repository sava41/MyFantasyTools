...
class ViewDataSimple : public mft::ViewData {
 public:
  ...
  bool load_camera_data(const std::array<float, MAT4_SIZE>& transform,
                        int sizex, int sizey, float fov) override {
    // Do nothing
    return true;
  }

  void* create_image_buffer(int sizex, int sizey, int channels,
                            size_t bufferSize, const ImageType& type) override {
    switch (type) {
...
std::unique_ptr<mft::ViewData> createViewDataSimple(
    const std::string& name, const std::filesystem::path& dataDir,
