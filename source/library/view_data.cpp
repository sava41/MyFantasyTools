...
ViewData::ViewData(const std::string& name,
                   const std::filesystem::path& dataDir,
                   unsigned int imageTypesFlags)
    : m_name(name), m_imageTypeFlags(imageTypesFlags) {
  for (const auto& type : ImageTypeStrings) {
    unsigned int typeInt = static_cast<unsigned int>(type.first);
    if (typeInt & imageTypesFlags) {
      m_imagePaths.insert(
          {type.first,
           dataDir / std::filesystem::path(name + "_" + type.second + ".jxl")});
    }
  }
}

ViewData::~ViewData() { unload_image_data(); }

bool ViewData::load_image_data() {
  if (m_imagesLoaded) return false;

  for (auto& image : m_imagePaths) {
    std::vector<char> compressed = read_binary(image.second);
    bool res = jxl::decode_oneshot(
        compressed,
        [this, &image](int sizex, int sizey, int channels,
                       size_t bufferSize) -> void* {
          void* bufferPtr = create_image_buffer(sizex, sizey, channels,
                                                bufferSize, image.first);
