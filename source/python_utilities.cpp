...
namespace mft::python {
    ...
    bool EncodeJxlOneshot(const uint32_t xsize, const uint32_t ysize, const uint32_t channels, 
    const void* pixel_data, std::vector<uint8_t>& compressed) {
        ...
        JxlPixelFormat pixel_format = {channels, JXL_TYPE_FLOAT, JXL_NATIVE_ENDIAN, 0};
    ...
    bool SaveJxl(const uint32_t xsize, const uint32_t ysize, const uint32_t channels, py::array_t<float> pixel_data, const std::string& path)
    {
        ...
        if(!EncodeJxlOneshot(xsize, ysize, channels, static_cast<const void*>(pixel_data_raw.data(0,0,0)), compressed)) {
            return false;
        }
